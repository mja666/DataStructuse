...
from fleet_visual import _flatten_route_points, _vehicle_xy
...
def _build_animation_runtime(
    ...
    duration_limit = max(0.5, min(float(duration_s), float(cfg.sim_duration)))
    sim_t = 0.0
...
def stream_live_canvas_frames(
    ...
    if last is not None:
        score_img = _render_score_curve_image(score_times, score_values)
        yield last, score_img, _summary_text(base, n_frames, media="live", writer_used="canvas")
...
def build_gradio_app(fleet_osm_mod: Any, fleet_osm_path: Optional[str]):
    ...
    with gr.Blocks(title="OSM Fleet Animation") as demo:
        gr.Markdown("## 车队调度 OSM 动画面板")
        gr.Markdown(
        ...
        summary = gr.Textbox(label="运行信息", lines=4)
